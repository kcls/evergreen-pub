-- Revert kcls-evergreen:XXXX-collection-hq-refresh from pg

BEGIN;

CREATE OR REPLACE FUNCTION collectionhq.attempt_price(text) RETURNS text
...
CREATE OR REPLACE FUNCTION collectionhq.attempt_year(text) RETURNS text
    LANGUAGE plpgsql STABLE STRICT
    AS $_$
  DECLARE
    attempt_value ALIAS FOR $1;
...
CREATE OR REPLACE FUNCTION collectionhq.quote(text) RETURNS text
    LANGUAGE plpgsql STABLE STRICT
    AS $_$
  DECLARE
    value ALIAS FOR $1;
...
CREATE OR REPLACE FUNCTION collectionhq.write_bib_rows_to_stdout(text, integer) RETURNS text
    LANGUAGE plpgsql
    AS $_$
...
CREATE OR REPLACE FUNCTION collectionhq.write_item_rows_to_stdout(text, integer) RETURNS text
    LANGUAGE plpgsql
    AS $_$
...
CREATE OR REPLACE FUNCTION collectionhq.write_item_rows_to_stdout_new(text, integer) RETURNS text
    LANGUAGE plpgsql
    AS $_$
