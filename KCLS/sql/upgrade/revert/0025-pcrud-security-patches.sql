-- Revert kcls-evergreen:0025-pcrud-security-patches from pg

BEGIN;

-- actor.org_unit_ancestors/descendants were already STABLE before the deploy;
-- no volatility change needed.
...
-- Restore public.extract_marc_field (4-arg); the 3-arg variant did not exist pre-upgrade.
CREATE OR REPLACE FUNCTION public.extract_marc_field ( TEXT, BIGINT, TEXT, TEXT ) RETURNS TEXT AS $function$
    SELECT regexp_replace(string_agg(output,' '),$4,'','g') FROM oils_xpath_table('id', 'marc', $1, $3, 'id='||$2)x(id INT, output TEXT);
$function$ LANGUAGE SQL;

-- Drop evergreen.extract_marc_field functions added by the deploy.
DROP FUNCTION IF EXISTS evergreen.extract_marc_field(TEXT, BIGINT, TEXT, TEXT);
DROP FUNCTION IF EXISTS evergreen.extract_marc_field(TEXT, BIGINT, TEXT);

-- Restore unapi.memoize (dropped by deploy).
CREATE OR REPLACE FUNCTION unapi.memoize (
    classname     TEXT,
    obj_id        BIGINT,
    format        TEXT,
    ename         TEXT,
...
-- Restore evergreen.change_db_setting (dropped by deploy).
CREATE OR REPLACE FUNCTION evergreen.change_db_setting ( setting_name TEXT, settings TEXT[] ) RETURNS VOID AS $function$
BEGIN
    EXECUTE 'ALTER DATABASE ' || quote_ident(current_database()) || ' SET ' || quote_ident(setting_name) || ' = ' || array_to_string(settings, ',');
END;
$function$ LANGUAGE plpgsql;

-- Drop the CHECK constraint added to config.metabib_class.
-- The constraint name is auto-generated by PostgreSQL; look it up dynamically.
DO $$
DECLARE
    cname TEXT;
BEGIN
    SELECT conname INTO cname
      FROM pg_constraint
     WHERE conrelid = 'config.metabib_class'::regclass
       AND contype = 'c'
       AND pg_get_constraintdef(oid) LIKE '%^\w+$%';
    IF cname IS NOT NULL THEN
        EXECUTE 'ALTER TABLE config.metabib_class DROP CONSTRAINT ' || quote_ident(cname);
    END IF;
END;
$$;
...
CREATE OR REPLACE FUNCTION search.symspell_suggest (
    ...
    max_ed          INT DEFAULT NULL,
    verbosity       INT DEFAULT NULL,
    skip_correct    BOOL DEFAULT NULL,
    max_word_opts   INT DEFAULT NULL,
    count_threshold INT DEFAULT NULL
) RETURNS SETOF search.symspell_lookup_output AS $F$
DECLARE
