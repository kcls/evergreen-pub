...
import {throwError, Observable, from, concat, empty} from 'rxjs';
...
export class LineitemService {
    ...
    getFleshedLineitems(ids: number[],
        ...
        return this.net.request(
            ...
            if (this.evt.parse(liStruct) || this.evt.parse(liStruct.lineitem)) {
                console.warn('Could not retrieve lineitem: ', liStruct);
                return throwError('Could not retrieve lineitem');
            }
            return this.ingestLineitem(liStruct, params.toCache);
        }));
    }
    ...
    applyBatchNote(liIds: number[],
        ...
        return this.net.request('open-ils.acq',
            ...
            if (resp && resp.note) {
                const cache = this.liCache[resp.note.lineitem()];
                if (cache) { // Could be on a not-fetched page
                    const li = cache.lineitem;
